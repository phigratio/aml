...
/**
 * RULE 045: Successful Transactions to the Creditor, Including New One.
 */
public class Rule045SuccessfulTransactionsToCreditorIncludingNewOne implements Rule {

    private static final String RULE_ID = "045";
    private static final String RULE_NAME = "Rule 045: Successful Transactions to the Creditor, Including New One";
    private static final String TYPOLOGY = "Increased Inflow Frequency / Accumulation";
    private static final String REGULATORY_BASIS =
        "Tazama Rule 045 pattern; FATF abnormal incoming frequency indicators";
    private static final int RULE_PRIORITY = 71;
    ...
    public RuleResult evaluate(Transaction transaction, CustomerContext customer, RuleContext context) {
        ...
        List<Transaction> currentTxs = context.transactionHistoryProvider().findTransactions(
            transaction.accountNumber(), currentStart, transaction.transactionDate(), Transaction.TransactionDirection.CREDIT
        );
        int currentCount = currentTxs.size() + 1;

        List<Transaction> historicalTxs = context.transactionHistoryProvider().findTransactions(
            transaction.accountNumber(), historicalStart, historicalEnd, Transaction.TransactionDirection.CREDIT
        );
        ...
        evidence.put("current_incoming_count", currentCount);
        ...
        return RuleResult.builder()
            ...
            .recommendedAction(triggered
                ? "Investigate sudden increase in creditor incoming transaction frequency"
                : "No immediate action. Continue monitoring creditor incoming transaction frequency")
